            ...
            >
              <el-row>
                ...
                <el-form-item
                  ...
                  :rules="[{ required: true, message: 'mysql不能为空' }]"
                >
                  <el-input
                    v-model="gatewayFrom.selfMysql.mysqlDataBase"
                ...
                >
                  <el-input
                    v-model="gatewayFrom.selfMysql.mysqlHost"
                ...
                >
                  <el-input
                    v-model.number="gatewayFrom.selfMysql.mysqlPort"
                ...
                >
                  <el-input
                    v-model="gatewayFrom.selfMysql.mysqlUser"
                ...
                >
                  <el-input
                    v-model="gatewayFrom.selfMysql.mysqlPwd"
                ...
                >
                  <el-input
                    v-model="gatewayFrom.selfRabbitMq.mqHost"
                ...
                >
                  <el-input
                    v-model.number="gatewayFrom.selfRabbitMq.mqPort"
                ...
                >
                  <el-input
                    v-model="gatewayFrom.selfRabbitMq.mqUser"
                ...
                >
                  <el-input
                    v-model="gatewayFrom.selfRabbitMq.mqPwd"
                ...
                >
                  <el-input
                    v-model="gatewayFrom.eurekaHost"
                    placeholder="如：spring-cloud-discovery:8761"
                  ></el-input>
                </el-form-item>
                <el-form-item
                  label="redis地址："
                  prop="selfRedis.redisHost"
                  :rules="[
                ...
                >
                  <el-input
                    v-model="gatewayFrom.selfRedis.redisHost"
                    placeholder="请输入redis地址"
                  ></el-input>
                </el-form-item>
                <el-form-item
                  label="redis端口号："
                  prop="selfRedis.redisPort"
                  :rules="[
                    { required: true, message: 'redis端口号不能为空' },
                    { type: 'number', message: 'redis端口号必须为数字值' },
                    {
                      pattern: /^([0-9]|[1-9]\d|[1-9]\d{2}|[1-9]\d{3}|[1-5]\d{4}|6[0-4]\d{3}|65[0-4]\d{2}|655[0-2]\d|6553[0-5])$/g,
                      message: '范围需为（80～65535）',
                      trigger: 'blur',
                ...
                >
                  <el-input
                    v-model.number="gatewayFrom.selfRedis.redisPort"
                    placeholder="请输入redis端口号"
                  ></el-input>
                </el-form-item>
                <el-form-item label="redis密码：">
                  <el-input
                    v-model="gatewayFrom.selfRedis.redisPwd"
...
export default {
  ...
  methods: {
    ...
    clickPVCNamespace(judge) {
      if (judge) {
        this.projectResource.envId = this.gatewayFrom.envId;
        this.projectResource.projectNo = this.gatewayFrom.projectNo;
      ...
      if (
        ...
        getResourceSpaceNameInfo(
          ...
          if (r.content.cpu < parseInt(this.gatewayFrom.gatewayCpu)) {
            ...
            r.content.memory < parseInt(this.gatewayFrom.gatewayMemory)
          ) {
            this.gatewayFrom.nameSpace = "";
          }
        });
    ...
    async commitOrder() {
      ...
      if (r.content.name == "网关服务标准版") {
        this.gatewayFrom.componentResourceLevel = 1;
        this.gatewayFrom.selfMysql.mysqlResourcelevel = 1;
        this.gatewayFrom.selfRabbitMq.mqResourcelevel = 1;
        this.gatewayFrom.selfRedis.redisResourcelevel = 1;
      }
      if (r.content.name == "网关服务高级版") {
        this.gatewayFrom.componentResourceLevel = 2;
        this.gatewayFrom.selfMysql.mysqlResourcelevel = 2;
        this.gatewayFrom.selfRabbitMq.mqResourcelevel = 2;
        this.gatewayFrom.selfRedis.redisResourcelevel = 2;
      }
      if (r.content.name == "网关服务企业版") {
        this.gatewayFrom.componentResourceLevel = 3;
        this.gatewayFrom.selfMysql.mysqlResourcelevel = 3;
        this.gatewayFrom.selfRabbitMq.mqResourcelevel = 3;
        this.gatewayFrom.selfRedis.redisResourcelevel = 3;
      }
      ...
      for (let a = 0; a < arr.length; a++) {
        ...
        if (arr1[0].trim() == "集群") {
          params.name = arr1[0];
          params.paramValue = this.getClustersLabel(this.gatewayFrom.envId);
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "项目名称") {
          params.name = arr1[0];
          params.paramValue = this.getObjectName(this.gatewayFrom.projectNo);
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "资源空间") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.nameSpace;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "mysql地址") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfMysql.mysqlHost;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "mysql端口号") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfMysql.mysqlPort;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "mysql用户名") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfMysql.mysqlUser;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "mysql密码") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfMysql.mysqlPwd;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "mysql数据库") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfMysql.mysqlDataBase;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "mysql资源等级") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfMysql.mysqlResourcelevel;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "Rabbitmq地址") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfRabbitMq.mqHost;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "Rabbitmq端口号") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfRabbitMq.mqPort;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "Rabbitmq用户名") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfRabbitMq.mqUser;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "Rabbitmq密码") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfRabbitMq.mqPwd;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "Rabbitmq资源等级") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfRabbitMq.mqResourcelevel;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "Redis资源等级") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfRedis.redisResourcelevel;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "redis地址") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfRedis.redisHost;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "redis端口号") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfRedis.redisPort;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "Redis密码") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.selfRedis.redisPwd;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "组件资源等级") {
          params.name = arr1[0];
          params.paramValue = this.gatewayFrom.componentResourceLevel;
          this.skuInfoSpecs.push(params);
        }
      }
      if (this.disable == true) {
        ...
        sessionStorage.setItem("gatewayfrom", JSON.stringify(this.gatewayFrom));
    ...
    async getClusters() {
      ...
      this.gatewayFrom.envId = this.edit[0].value;
    },
    //获取项目信息下拉框
    async getProject1() {
      //获取
