          ...
          <el-col :offset="2" :span="18">
            ...
            >
              <el-row>
                ...
                >
                  <el-input
                    v-model.number="monitoringFrom.selfRabbitMq.mqPort"
                ...
                >
                  <el-input
                    v-model="monitoringFrom.selfRabbitMq.mqUser"
                ...
                >
                  <el-input
                    v-model="monitoringFrom.selfRabbitMq.mqPwd"
                ...
                <el-form-item
                  label="服务降级镜像："
                  prop="nameSpace"
                  :rules="[{ required: true, message: '服务降级镜像不能为空' }]"
                >
                  <el-select
                    v-model="serviceDegradation"
                    placeholder="请选择服务降级"
                    style="width: 100%"
                  >
                    <el-option
                      v-for="item in serviceDegradations"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    ></el-option>
                ...
                <el-form-item
                  label="熔断监控镜像："
                  prop="nameSpace"
                  :rules="[{ required: true, message: '熔断监控镜像不能为空' }]"
                >
                  <el-select
                    v-model="fuseMonitoring"
                    placeholder="请选择熔断监控"
                    style="width: 100%"
                  >
                    <el-option
                      v-for="item in fuseMonitorings"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    ></el-option>
...
export default {
  ...
  data: function () {
    return {
      ...
      serviceDegradation: "spring-cloud-hystrix-degrade:Edgware-SR6",
      fuseMonitoring: "spring-cloud-hystrix-turbine:Edgware-SR6",
      serviceDegradations: [
        {
          value: "spring-cloud-hystrix-degrade:Edgware-SR6",
          label: "spring-cloud-hystrix-degrade:Edgware-SR6",
        },
      ],
      fuseMonitorings: [
        {
          value: "spring-cloud-hystrix-turbine:Edgware-SR6",
          label: "spring-cloud-hystrix-turbine:Edgware-SR6",
        },
      ],
      disable: false,
      sum: 0,
  ...
  methods: {
    ...
    //验证组件是否被注册
    verification(data) {
      let component = "spring-cloud-hystrix-turbine";
    ...
    async rowClick(row, column, event) {
      ...
      this.monitoringFrom.degradeCpu = parseFloat(row.cpuCores) + "";
      this.monitoringFrom.degradeMemory = parseFloat(row.memory) + "";

      this.monitoringFrom.turbineCpu = parseFloat(row.cpuCores) + "";
      this.monitoringFrom.turbineMemory = parseFloat(row.memory) + "";
      if (
        ...
        getResourceSpaceNameInfo(
          ...
          const messageto =
            ...
            memory +
            "G）";
          if (r.content.cpu < parseInt(this.monitoringFrom.degradeCpu * 2)) {
            this.$notify({
              type: "warning",
              message: message,
            });
            ...
            r.content.memory < parseInt(this.monitoringFrom.degradeMemory * 2)
          ) {
            this.$notify({
              type: "warning",
              message: message,
            ...
            r.content.cpu == parseInt(this.monitoringFrom.degradeCpu * 2)
          ) {
            this.$notify({
              type: "warning",
              message: messageto,
    ...
    async commitOrder() {
      ...
      params2.name = "集群";
      params2.paramValue = this.getClustersLabel(this.monitoringFrom.envId);
      ...
      params4.name = "资源空间";
      params4.paramValue = this.monitoringFrom.nameSpace;
      this.skuInfoSpecs.push(params4);
      let params5 = { name: "", paramValue: "" };
      params5.name = "mysql地址";
      params5.paramValue = this.monitoringFrom.selfMysql.mysqlHost;
      this.skuInfoSpecs.push(params5);
      let params6 = { name: "", paramValue: "" };
      params6.name = "Rabbitmq地址";
      params6.paramValue = this.monitoringFrom.selfRabbitMq.mqHost;
      ...
      //     params.name = arr1[0];
      //     params.paramValue = this.monitoringFrom.selfRabbitMq.mqPort;
      //     this.skuInfoSpecs.push(params);
      //   }
      //   if (arr1[0].trim() == "Rabbitmq用户名") {
      //     params.name = arr1[0];
      //     params.paramValue = this.monitoringFrom.selfRabbitMq.mqUser;
      //     this.skuInfoSpecs.push(params);
      //   }
      //   if (arr1[0].trim() == "Rabbitmq密码") {
      //     params.name = arr1[0];
      //     params.paramValue = this.monitoringFrom.selfRabbitMq.mqPwd;
      //     this.skuInfoSpecs.push(params);
      //   }
      ...
      if (this.disable == true) {
        ...
        this.monitoringFrom.degradeCpu = (
          this.monitoringFrom.degradeCpu * 1
        ).toFixed(1);

        this.monitoringFrom.degradeMemory = (
          this.monitoringFrom.degradeMemory * 1
        ).toFixed(1);

        this.monitoringFrom.turbineCpu = (
          this.monitoringFrom.turbineCpu * 1
        ).toFixed(1);

        this.monitoringFrom.turbineMemory = (
          this.monitoringFrom.turbineMemory * 1
        ).toFixed(1);
    ...
    async fetchData() {
      ...
      this.monitoringFrom.degradeCpu = parseFloat(list[0].cpuCores) + "";
      this.monitoringFrom.degradeMemory = parseFloat(list[0].memory) + "";

      this.monitoringFrom.turbineCpu = parseFloat(list[0].cpuCores) + "";
      this.monitoringFrom.turbineMemory = parseFloat(list[0].memory) + "";
    },
    //获取集群
    async getClusters() {
      this.edit = [];
