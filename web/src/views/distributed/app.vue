      ...
      <el-col :offset="3" :span="18" style="background: #FFF;">
        ...
        <el-row>
          ...
          <el-col :span="10" style="margin-top:30px;margin-bottom:30px;">
            ...
            <span class="specFont"
              >{{ skuData.cpu * 2 }}Core/{{ skuData.gb * 2 }}GB</span
            >
          </el-col>
        </el-row>
      </el-col>
          ...
          <el-col :offset="2" :span="18">
            ...
            >
              <el-row>
                ...
                >
                  <el-input
                    v-model="formdata.selfMysql.mysqlDataBase"
                ...
                >
                  <el-input
                    v-model="formdata.selfMysql.mysqlHost"
                ...
                >
                  <el-input
                    v-model.number="formdata.selfMysql.mysqlPort"
                ...
                >
                  <el-input
                    v-model="formdata.selfMysql.mysqlUser"
                ...
                >
                  <el-input
                    v-model="formdata.selfMysql.mysqlPwd"
                ...
                >
                  <el-input
                    v-model="formdata.selfRedis.redisHost"
                ...
                >
                  <el-input
                    type="age"
                    v-model.number="formdata.selfRedis.redisPort"
                    placeholder="请输入redis端口号"
                  ></el-input>
                </el-form-item>
                <el-form-item label="redis密码：">
                  <el-input
                    v-model="formdata.selfRedis.redisPwd"
...
export default {
  ...
  methods: {
    ...
    clickPVCNamespace(judge) {
      if (judge) {
        this.projectResource.envId = this.formdata.envId;
        this.projectResource.projectNo = this.formdata.projectNo;
    ...
    //验证组件是否被注册
    verification(data) {
      let component = "spring-cloud-dtm";
      registration(this.formdata.envId, component, data).then(r => {
        if (r.content == 1) {
      ...
      if (this.formdata.nameSpace != null && this.formdata.nameSpace != "") {
        getResourceSpaceNameInfo(
          ...
          const messageto =
            ...
            memory +
            "G）";
          if (r.content.cpu < parseInt(this.formdata.dtmCpu * 2)) {
            this.$notify({
              type: "warning",
              message: message
            });
            this.formdata.nameSpace = "";
          } else if (r.content.memory < parseInt(this.formdata.dtmMemory * 2)) {
            this.$notify({
              type: "warning",
              message: message
            });
            this.formdata.nameSpace = "";
          } else if (r.content.cpu == parseInt(this.formdata.dtmCpu * 2)) {
            this.$notify({
              type: "warning",
              message: messageto
            });
            ...
            r.content.memory == parseInt(this.formdata.dtmMemory * 2)
          ) {
            this.$notify({
              type: "warning",
              message: messageto
    ...
    async commitOrder() {
      ...
      if (r.content.name == "分布式服务（标准版）") {
        this.formdata.componentResourceLevel = 1;
        this.formdata.selfMysql.mysqlResourcelevel = 1;
        this.formdata.selfRedis.redisResourcelevel = 1;
      }
      if (r.content.name == "分布式服务（高级版）") {
        this.formdata.componentResourceLevel = 2;
        this.formdata.selfMysql.mysqlResourcelevel = 2;
        this.formdata.selfRedis.redisResourcelevel = 2;
      }
      if (r.content.name == "分布式服务（企业版）") {
        this.formdata.componentResourceLevel = 3;
        this.formdata.selfMysql.mysqlResourcelevel = 3;
        this.formdata.selfRedis.redisResourcelevel = 3;
      }
      ...
      for (let a = 0; a < arr.length; a++) {
        ...
        if (arr1[0].trim() == "区域信息") {
          params.name = arr1[0];
          params.paramValue = this.regionalInformation;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "项目名称") {
          params.name = arr1[0];
          params.paramValue = this.getObjectName(this.formdata.projectNo);
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "mysql地址") {
          params.name = arr1[0];
          params.paramValue = this.formdata.selfMysql.mysqlHost;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "mysql端口号") {
          params.name = arr1[0];
          params.paramValue = this.formdata.selfMysql.mysqlPort;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "mysql用户名") {
          params.name = arr1[0];
          params.paramValue = this.formdata.selfMysql.mysqlUser;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "mysql密码") {
          params.name = arr1[0];
          params.paramValue = this.formdata.selfMysql.mysqlPwd;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "mysql") {
          params.name = arr1[0];
          params.paramValue = this.formdata.selfMysql.mysqlDataBase;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "redis主机地址(域名)") {
          params.name = arr1[0];
          params.paramValue = this.formdata.selfRedis.redisHost;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "redis端口号") {
          params.name = arr1[0];
          params.paramValue = this.formdata.selfRedis.redisPort;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "连接密码") {
          params.name = arr1[0];
          params.paramValue = this.formdata.selfRedis.redisPwd;
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "连接密码") {
          params.name = arr1[0];
        ...
        if (arr1[0].trim() == "集群") {
          params.name = arr1[0];
          params.paramValue = this.getClustersLabel(this.formdata.envId);
          this.skuInfoSpecs.push(params);
        }
        if (arr1[0].trim() == "注册中心地址") {
          params.name = arr1[0];
          params.paramValue = this.formdata.eurekaHost;
          this.skuInfoSpecs.push(params);
        }
      }
      if (this.disable == true) {
        ...
        this.formdata.dtmCpu = (this.formdata.dtmCpu * 1).toFixed(1);

        this.formdata.dtmMemory = (this.formdata.dtmMemory * 1).toFixed(1);
        this.formdata.taskCpu = (this.formdata.taskCpu * 1).toFixed(1);

        this.formdata.taskMemory = (this.formdata.taskMemory * 1).toFixed(1);
    ...
    async fetchData() {
      ...
      this.search.params = `[{"param":{"catalogId":${a}},"sign":"EQ"}]`;
      this.search.page = 1;
    ...
    async getClusters() {
      ...
      this.formdata.envId = this.edit[0].value;
    },
    //获取项目信息下拉框
    async getProject1() {
      //获取
