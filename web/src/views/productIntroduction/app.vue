  ...
  <div id="app" style>
    ...
    >
      <el-col :span="10" :offset="2">
        <el-col class="header1-title">{{ list.name }}</el-col>
        <el-col class="header1-detail" :span="24">{{ list.productintroduction }}</el-col>
        <el-col>
          <el-button class="header1-btn" @click="linkBuy()">立即申请</el-button>
          <el-button class="header1-btn1" @click="jump1(list.id, productId, 2)">查看详情</el-button>
        </el-col>
      </el-col>
      <el-col :span="10" :offset="2">
        <div class="header1-logo" style="position:relative;top:185px;left:0px;">
          <img style="width:300px;" :src="baseURL.DATAURL + list.logo.url" alt />
        </div>
      </el-col>
    </el-row>
    <el-row style="background-color: white;height:70px;">
      <el-col :span="20" :offset="2">
        ...
        >
          <p class="jump1" @click="jump1(item.productId, item.id, index)">{{ item.title }}</p>
        </el-col>
      </el-col>
    </el-row>
    <div>
      <el-row style="margin-top:50px;">
        ...
        <el-col :span="5" :offset="1">
          ...
          <div class="fontChiese" style="margin-top:55px;margin-bottom: 55px">{{ list.name }}</div>
        </el-col>
        <!--  Center  -->
  ...
  methods: {
    linkBuy() {
      if (getAccessToken() != undefined) {
        window.location.href = `/html/serviceTemplate.html?productId=${this.servicecatalogId}`;
      } else {
    ...
    jump1(id, itemid, index) {
      ...
      window.location.href = `/html/productDetails.html?id=${this.id}&productId=${this.productId}&ll=${itemid}&productName=${this.productName}&catalogId=${this.catalogId}&catalog=${this.catalog}`;
    },
    getId(name) {
      return (
        decodeURIComponent(
