        ...
        <el-col :offset="2">
          ...
          >
            <el-form-item
              label="区域信息："
              prop="envId"
              :rules="[{ required: true, message: '区域信息不能为空'}]"
            >
              <el-select
  ...
  methods: {
    ...
    async commitOrder() {
      ...
      for (var i = 0; i < this.skuInfoSpecs.length; i++) {
        ...
        if (this.skuInfoSpecs[i].name == "nexus地址") {
          this.skuInfoSpecs[i].paramValue = this.monitoringFrom.nexus_url;
